...
def get_final_preds(batch_heatmaps: torch.Tensor,
    ...
    batch_size = batch_heatmaps.shape[0]  # 从batch_heatmaps中获取批次大小
    num_joints = coords.shape[1]          # 获取关节点数量
    ...
    # return preds, maxvals.cpu().detach().numpy()


def decode_keypoints(outputs, origin_hw, num_joints: int = 24):
    keypoints = []
    scores = []
...
class AffineTransform(object):
    ...
    def __init__(self,
                 ...
                 #fixed_size: Tuple[int, int] = (1024, 768)):
...
class KeypointToHeatMap(object):
    def __init__(self,
                 ...
                 #heatmap_hw: Tuple[int, int] = (1920 // 4, 1088 // 4),
    ...
    def __call__(self, image, target):
        ...
        # 保留腿长信息
        if "leg_lengths" in target:
            # 确保leg_lengths是torch.Tensor类型
            if not isinstance(target["leg_lengths"], torch.Tensor):
                target["leg_lengths"] = torch.as_tensor(target["leg_lengths"], dtype=torch.float32)
