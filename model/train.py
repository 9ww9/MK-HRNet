...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        torch.save(save_files, "E:/4.weight/2025.08.29/weight/model-{}.pth".format(epoch))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--fixed-size', default=[256, 192], nargs='+', type=int, help='input size')  # 256, 192
    # keypoints点数
    parser.add_argument('--num-joints', default=24, type=int, help='num_joints')
    # 文件保存地址
    parser.add_argument('--output-dir', default='E:/zanshiweight/', help='path where to save')
    # 若需要接着上次训练，则指定上次训练保存权重文件地址
    parser.add_argument('--resume', default='', type=str, help='resume from checkpoint')
    # 指定接着从哪个epoch数开始训练
    parser.add_argument('--start-epoch', default=0, type=int, help='start epoch')
    # 训练的总epoch数
    parser.add_argument('--epochs', default=601, type=int, metavar='N',
                        help='number of total epochs to run')
    # 针对torch.optim.lr_scheduler.MultiStepLR的参数
    parser.add_argument('--lr-steps', default=[170, 200], nargs='+', type=int,
                        help='decrease lr every step-size epochs')
    # 针对torch.optim.lr_scheduler.MultiStepLR的参数
    parser.add_argument('--lr-gamma', default=1, type=float, help='decrease lr by a factor of lr-gamma')
    # 学习率
