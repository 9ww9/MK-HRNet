...
def summarize(self, catId=None):
    ...
    def _summarize(ap=1, iouThr=None, areaRng='all', maxDets=10):
        p = self.params
        iStr = ' {:<18} {} @[ IoU={:<9} | area={:>6s} | maxDets={:>3d} ] = {:0.3f}'#Python字符串格式化
    ...
    stats[0], print_list[0] = _summarize(1, maxDets=10)
    stats[1], print_list[1] = _summarize(1, maxDets=10, iouThr=.5)
    stats[2], print_list[2] = _summarize(1, maxDets=10, iouThr=.75)
    stats[3], print_list[3] = _summarize(1, maxDets=10, areaRng='medium')
    stats[4], print_list[4] = _summarize(1, maxDets=10, areaRng='large')
    stats[5], print_list[5] = _summarize(0, maxDets=10)
    stats[6], print_list[6] = _summarize(0, maxDets=10, iouThr=.5)
    stats[7], print_list[7] = _summarize(0, maxDets=10, iouThr=.75)
    stats[8], print_list[8] = _summarize(0, maxDets=10, areaRng='medium')
    stats[9], print_list[9] = _summarize(0, maxDets=10, areaRng='large')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--resize-hw', type=list, default=[256, 192], help="resize for predict")#256,192
    # 是否开启图像翻转
